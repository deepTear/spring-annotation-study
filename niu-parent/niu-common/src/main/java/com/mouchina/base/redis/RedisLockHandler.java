...
public class RedisLockHandler {
	private static Logger logger = LogManager.getLogger("RedisLockHandler");
...
//			returnBrokenResource(pool, jedis);
	 ...
	 * 
	 * @param key
	 * @param timeout 获取不到的话就放弃的时间(毫秒)
	 * @param millis 每隔多久获取一次(假如始终未获取到)()
	 * @return
	 */
		...
		try {
			...
			do {
				...
				if (i == 1) {
					...
					logger.info("Redis->tryLock获得本次锁总共耗时 : " + (endTime-startTime) + "毫秒");
					return Boolean.TRUE;
				} else { // 存在锁
...
//			returnBrokenResource(pool, jedis);
	 ...
	 * @param jedis
	 */
	private void returnResource(ShardedJedisPool pool, ShardedJedis sjedis) {
//		if (jedis == null) {
//			return;
//		}
//
