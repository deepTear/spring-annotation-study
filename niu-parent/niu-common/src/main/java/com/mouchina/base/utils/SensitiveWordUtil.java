...
public class SensitiveWordUtil {
    ...
    public static Map<String,String> sensitiveWordMap2;
    ...
    public static synchronized void init(Map<String,String> sensitiveWordMap) {
    	sensitiveWordMap2 = sensitiveWordMap;
        initSensitiveWordMap(sensitiveWordMap);
    }
    ...
    private static void initSensitiveWordMap(Set<String> sensitiveWordSet) {
        ...
        while (iterator.hasNext()) {
            //关键字
        	//TODO
            key = iterator.next().split("#SENSITIVE_WORD_SPLIT#")[0];
     ...
     * @param sensitiveWordSet 敏感词库
     */
    private static void initSensitiveWordMap(Map<String,String> sensitiveWordMap_) {
        //初始化敏感词容器，减少扩容操作
        sensitiveWordMap = new HashMap(sensitiveWordMap_.size());
        String key;
        Map nowMap;
        Map<String, String> newWorMap;
        //迭代sensitiveWordSet
        Iterator<String> iterator = sensitiveWordMap_.keySet().iterator();
        while (iterator.hasNext()) {
            //关键字
        	//TODO
            key = iterator.next();
...
*/
    ...
    public static String replaceSensitiveWord(String txt,int matchType) {
        ...
        Iterator<String> iterator = set.iterator();
        String word;
        String replacePattern_ = "***";
        String replacePattern;
        while (iterator.hasNext()) {
            word = iterator.next();
            if(sensitiveWordMap2 != null && sensitiveWordMap2.size() > 0){
            	replacePattern = sensitiveWordMap2.get(word);
            	if(StringUtils.isNotEmpty(replacePattern)){
            		replacePattern_ = replacePattern;
            	}
            }
            resultTxt = resultTxt.replaceAll(word, replacePattern_);
        }
    ...
    public static void main(String[] args) {
        ...
        try {
            in = SensitiveWordUtil.class.getClassLoader().getResourceAsStream("word/words.dict");
            reader = new InputStreamReader(in);
            breader = new BufferedReader(reader);
            String readLine = "";
            while((readLine = breader.readLine()) != null){
            	if(StringUtils.isNotBlank(readLine) && !readLine.endsWith(":")){
            		sensitiveWordSet.add(readLine);
            	}
            }
        ...
        String string = "太多的伤感情怀也许只局限于饲养基地 荧幕中的情节。"
                + "然后我们的扮演的角色就是跟随着主人公的jiangzemin喜红客联盟 怒哀乐而过于牵强的把自己的情感也附加于银幕情节中，然后感动就流泪，"
                + "难过就躺在某一个人的怀里尽情的阐述心扉或者手机卡复制器一个贱人一杯红酒一部电习近平影在夜 深人静的晚上，关上电话静静的发呆着。";
